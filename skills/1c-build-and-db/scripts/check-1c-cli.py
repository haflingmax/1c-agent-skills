...
K_VALUE_LOST = "K022"          # значением ключа стал другой ключ — значение потеряно
K_UNBALANCED_QUOTE = "K023"    # нечётное число кавычек — строка искажена по дороге
...
def split_args(line):
    r"""Разбирает строку с учётом кавычек, не ломаясь о пути с пробелами.
    ...
    Строка документации помечена префиксом r, и это не вкус: без него
    `\S` в тексте выше даёт SyntaxWarning в stderr, а PowerShell 5.1
    объявляет любой stderr нативной программы отказом. Так прибор шесть
    раз подряд «падал» с кодом 0 в живом прогоне 25.09.2026, и модель
    ушла писать свой запускатель. Сторож —
    tests/test_scripts_are_quiet_on_stderr.py.
    """
    аргументы = []
    текущий = ""
    в_кавычках = False
...
def check(line, catalog, ответы=frozenset()):
    ...
    `line` — строка ИЛИ уже готовый список аргументов. Список надёжнее
    и не требует разбора вовсе: склейка списка в строку воссоздаёт ту самую
    двусмысленность, ради устранения которой список и заводится —
    «C:/Program Files/…/1cv8.exe» после склейки снова распадается
    по пробелу, args[0] становится «C:/Program», и прибор отвечает K017
    «вне компетенции», то есть молча не проверяет ничего. Поймано живой
    проверкой 25.09.2026.
    ...
    """
    keys = catalog["ключи"]
    args = list(line) if not isinstance(line, str) else split_args(line)
    problems = []
    notes = []
    ...
    # Незакрытая кавычка — строка искажена по дороге, и разбирать её дальше
    # значит проверять не ту команду, что написана. Проверено печатью
    # sys.argv 25.09.2026: PowerShell 5.1, передавая строку нативной
    # программе, превращает «""» внутри неё в одну кавычку. Разбор склеивал
    # весь хвост в значение /P, не видел /DumpCfg и /DisableStartupDialogs
    # и отвечал кодом 0 — «можно запускать» про команду, которую не прочёл.
    if isinstance(line, str) and line.count('"') % 2:
        return ["%s в строке нечётное число кавычек — она искажена по дороге, "
                "и проверять её дальше значило бы проверять не ту команду. "
                "PowerShell 5.1 превращает \"\" внутри строки в одну кавычку. "
                "Передай команду списком аргументов после --, а пустой пароль "
                "— флагом --пустой-пароль обёртки run-1c.py" % K_UNBALANCED_QUOTE], []
    ...
    # Значение ключа, съеденное оболочкой. Правило общее и не про один /P:
    # ключ, которому документация предписывает обязательное значение
    # («arg» начинается с «<»), не может стоять вплотную к другому ключу.
    #
    # Блокировка доказана запуском 25.09.2026 на живой базе: PowerShell 5.1
    # молча выбрасывает пустой аргумент при вызове нативной программы, и
    # команда «… /N Администратор /P "" /LoadConfigFromFiles <каталог> …»
    # доезжает до платформы как «… /P /LoadConfigFromFiles <каталог> …».
    # Платформа берёт /LoadConfigFromFiles за пароль, ключа загрузки больше
    # нет, каталог остаётся позиционным аргументом и перебивает /F — ответ
    # «Неопределена информационная база», код 1, 1.1 с. Тот же argv, поданный
    # списком из Python, проходит вход и добирается до настоящей работы.
    #
    # Почему это отказ, а не замечание: пароль, равный имени ключа платформы,
    # — не тот случай, ради которого стоит пропускать доказанную порчу. Цена
    # молчания измерена: модель повторила один и тот же запуск восемь раз
    # за два с половиной часа и ушла писать свой запускатель.
    for name, i in used:
        требует = (keys[name].get("arg") or "").startswith("<")
        # Кавычки внутри аргумента здесь не снимаются: «/P""» — это ключ
        # с явно заданным ПУСТЫМ значением, законная документированная
        # форма, и она длиннее имени ключа. Снимая кавычки, прибор видел
        # бы голый «/P» и блокировал собственный пример из ПОДСКАЗКИ
        # обёртки — сторож test_no_command_from_skill_bodies_is_blocked
        # поймал это в тот же час.
        сырое = args[i]
        if len(сырое) > 2 and сырое[0] == '"' and сырое[-1] == '"':
            сырое = сырое[1:-1]
        слитно = len(сырое.split(":", 1)[0]) > len(name)
        if not требует or слитно:
            continue
        следом = args[i + 1].strip('"') if i + 1 < len(args) else None
        if следом is None:
            problems.append(
                "%s %s стоит последним, без значения (ожидается %s). Значение "
                "потеряно по дороге: так выглядит аргумент, выброшенный "
                "оболочкой" % (K_VALUE_LOST, name, keys[name]["arg"]))
            continue
        чужой = _канон(следом, keys)
        if чужой is not None:
            problems.append(
                "%s значением %s стал ключ %s (ожидается %s). Значение потеряно "
                "по дороге: PowerShell 5.1 молча выбрасывает пустой аргумент при "
                "вызове нативной программы — проверено запуском 25.09.2026. "
                "Пустой пароль задаётся ключом --пустой-пароль обёртки "
                "run-1c.py либо отсутствием /P вовсе, а не парой /P \"\""
                % (K_VALUE_LOST, name, следом, keys[name]["arg"]))
    ...
    # Для формы со списком строка собирается здесь и только для этой проверки —
    # на разбор аргументов она не влияет.
    целиком = line if isinstance(line, str) else " ".join(args)
    for pat, why in PLACEHOLDERS:
        m = pat.search(целиком)
        if m:
