...
(/L, /P, /N, /O, /WSA и прочие): самое длинное совпадение по префиксу берётся
среди ВСЕХ ключей каталога, и хвост, не вошедший в совпадение, платформа
отправляет дальше сама (проверено запуском дважды — на /DumpCfgToFile и на
/LoadCfgFromFile, docs/evidence/2026-08-22-blocking-rules.md). Признак glued
решает, куда уходит хвост: у /Lru (glued) хвост «ru» — легитимное слитное
значение, предупреждение. У /LoadCfgFromFile (не glued) хвост «FromFile»
платформа отправляет в позиционный аргумент — это уже проверено неверно
(«Файл не обнаружен»), поэтому блокировка.
...
def known_key(name, keys):
    ...
    Н-02: совпадение по префиксу ищется среди ВСЕХ ключей каталога, а не
    только среди двенадцати слитных, — платформа делает так же. Проверено
    запуском дважды: /DumpCfgToFile платформа разбирает как /DumpCfg (не как
    /D — такого ключа вовсе нет), а /LoadCfgFromFile — как /LoadCfg, а не как
    /L. Ограничивать поиск двенадцатью glued-ключами означало раньше, что
    /LoadCfgFromFile ложно резолвился в /L: короче настоящего совпадения
    платформы и с другим смыслом хвоста (docs/evidence/2026-08-22-blocking-rules.md).

    Среди всех совпадений берётся самое длинное, а не первое: /O и /OIDA оба
    подходят /OIDAOff, и результат обязан быть /OIDA, а не /O.
    ...
    for k in keys:
        if k.lower() == low:
            return k
    candidates = [k for k in keys if len(low) > len(k) and low.startswith(k.lower())]
...
def check(line, catalog):
    ...
    for i, a in enumerate(args):
        ...
        # берёт самое длинное совпадение среди ВСЕХ ключей и решает по-своему,
        # куда девать хвост. Признак glued резолвнутого ключа говорит, куда:
        tail = name[len(canon):]
        if keys[canon].get("glued"):
            ...
            # отправляет отдельно от ключа. Это уже проверено запуском дважды
            # и оба раза неверно: /DumpCfgToFile выгружает конфигурацию не в
            # тот файл (тихая порча), /LoadCfgFromFile падает кодом 1 «Файл
            # не обнаружен» (docs/evidence/2026-08-22-blocking-rules.md).
            # В отличие от glued-случая, здесь платформа доказанно не делает
            ...
            problems.append(
                ...
                "на /DumpCfgToFile и /LoadCfgFromFile)" % (a, canon, tail))
