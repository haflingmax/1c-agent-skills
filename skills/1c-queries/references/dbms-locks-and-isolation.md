...
- **как платформа отображает управляемые блокировки на блокировки СУБД.**
  Ни ИТС, ни документация СУБД этого соответствия не описывают:
  в технологическом журнале видно `TLOCK` платформы и `DBMSSQL`
  с текстом запроса, но не то, во что именно управляемая блокировка
  превратилась на стороне SQL Server;
