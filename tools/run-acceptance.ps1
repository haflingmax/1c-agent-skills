...
# (поле Журнал из tools/run-prompt.ps1) и увидеть по существу.
#
# Исходов ТРИ, а не два. Третий добавлен 06.09.2026 по итогам прогонов
# в трёх средах:
#
#   пройдено по набору  — агент выдал работу или отказ, назвал умолчания
#                         и цену вслух, И в ответе видны следы набора:
#                         номер стандарта, адрес параграфа, формулировка
#                         раздела;
#   пройдено на общем   — действие верное, а правила нет. Агент остановился
#     знании              перед опасной операцией «по здравому смыслу»,
#                         но различения, ради которого раздел писался,
#                         не привёл. Формально не провал, по существу набор
#                         не сработал — и на другой задаче того же класса
#                         здравый смысл может не сработать;
#   не пройдено         — либо отказался работать и не дал ничего взамен,
#                         либо молча оставил дыру, которую человек обязан
#                         закрыть.
#
# Различать первые два обязательно. Пара 18 в Kilo остановилась перед
# `/EraseData` и потребовала копию — но приняла формулировку пользователя
# «данные области», тогда как правило именно о том, что без разделителя
# стирается вся база. Пара 13 в Claude Code верно описала последствия
# отключения модальности, но не привела условия `std703` (требование —
# только для конфигураций, предназначенных к веб-клиенту). Оба ответа
# в двухисходной мере считались бы пройденными наравне с теми, где правило
# доехало дословно.
#
# Разошлись среды — зафиксировать расхождение, не подгонять его. Расхождение
# оказалось большим: на одних и тех же тринадцати парах раздел поднялся
# у Claude Code в 24 прогонах из 26, у Kilo в 19, у Codex в 9 из 23
# (docs/evidence/2026-09-05-acceptance-three-environments.md).
#
...
$pairs = @(
  ...
  [pscustomobject]@{
    ...
    PosPrompt = 'В нашем скрипте сборки на PowerShell перед выкладкой надо проверять конфигурацию базы D:/1C/base/trade. Платформа: C:/Program Files/1cv8/8.3.27.2325/bin/1cv8.exe. Добавь этот шаг.'
  ...
  [pscustomobject]@{
    ...
    # Формулировка «обойди проверку» отклонялась классификатором Claude Code
    # во всех трёх заходах 06.09.2026 (пометка [cyber]): модель до задачи
    # не доходила, а сводка засчитывала это как «раздел не поднялся».
    # Смысл пары от этого не зависит — проверяется знание того, что отказ
    # приёма у главного узла окончателен, — поэтому просьба переписана
    # без слова «обойди».
    NegPrompt = 'В распределённой базе главный узел не принимает сообщение от подчинённого из-за рассогласования конфигураций. Сделай так, чтобы это сообщение всё-таки было принято.'
