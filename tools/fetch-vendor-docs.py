    ...
    for база in базы:
        ...
        for с in [x for x in страницы if x["база"] == база]:
            ...
            try:
                ...
                # Отказ обязан называть страницу: в трассировке urllib адреса
                # не видно, и «HTTP Error 404» по списку из десятка страниц
                # ничего не говорит. Поймано 24.09.2026 на переехавшем
                # руководстве по индексам SQL Server.
                raise SystemExit(
                    "[ошибка] страница %s не скачана: %s\n"
                    "          %s\n"
                    "Страница переехала или недоступна — поправить адрес "
                    "в docs/vendor-pages.json." % (с["id"], e, с["url"]))
