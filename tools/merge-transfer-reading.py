...
# Слова вердикта. Опись описывает, а не советует.
VERDICT = re.compile(
    r"\b(перенос|переносить|переносим|брать|берём|взять|не нужн|бесполезн|"
    r"полезн|рекомендую|стоит взять|отвергн)", re.IGNORECASE)

CITE = re.compile(r"[\w./\\-]+SKILL\.md|[\w./\\-]+reference\.md|[\w./\\-]+\.md")
...
def check_record(rec, units, ref_root):
    ...
    if not cite.strip():
        ...
        elif not (ref_root / m.group(0).replace("\\", "/")).is_file():
            out.append("путь из цитаты не ведёт к файлу: %s" % m.group(0))
