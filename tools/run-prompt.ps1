...
if ($Env -eq 'claude') {
  ...
  # codex: структурного журнала нет вовсе — Codex пишет расшифровку разговора
  # прозой. Прежняя редакция искала здесь имя навыка подстрокой по всему
  # тексту, а этот приём объявлен негодным ещё 23.08.2026 (М-17/М-18,
  # docs/reviews/2026-08-23-core-review.md): он засчитывает любое упоминание,
  # включая рассуждение модели «стоило бы посмотреть 1c-security».
  #
  # Замер 05.09.2026 на прогоне из 23 задач показал, во что это обходится:
  # подстрочный признак дал «навык поднялся» в 20 прогонах, а файл навыка
  # модель открывала в 9. Завышение вдвое, и всё это время оно выглядело
  # как «в Codex набор работает лучше, чем в Kilo».
  #
  # Настоящий признак у Codex другой и структурный: навык он берёт не
  # инструментом, а **чтением файла** — в расшифровке видна команда оболочки
  # с путём вида ...\skills\1c-security\SKILL.md. Его и проверяем.
  # Подтверждение: все 19 ссылок на источник в том прогоне пришли из тех
  # девяти, где файл открывался, и ни одной из остальных четырнадцати.
  $navyk = $false
  $names = @()
  foreach ($имя in $skillNames) {
    $обр = 'skills[\\/]+' + [regex]::Escape($имя) + '[\\/]+SKILL\.md'
    if ($txt -match $обр) {
      $navyk = $true
      $names += $имя
    }
