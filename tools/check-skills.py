...
def check(skill_md):
    ...
    # Обратная сторона той же проверки: справочник, на который тело
    # не ссылается, агент не откроет никогда. Ссылки вниз запрещены
    # (см. выше — цепочка глубже одного уровня), поэтому единственная дверь
    # в справочник — тело, и незапертая дверь тут не бывает.
    #
    # Найдено 05.09.2026 на `1c-quality-gates`: `measuring-production.md`,
    # 129 строк о замере на работающей базе, не был назван ни телом,
    # ни любым другим файлом набора. Прибор молчал, потому что смотрел
    # только в сторону «ссылка → файл».
    # Ссылкой считается любое упоминание пути, а не только markdown-ссылка:
    # половина тел набора ведёт в конце опись вида «- `references/имя.md` —
    # о чём он», и это законная форма. Требование markdown-скобок объявило бы
    # недостижимыми девять живых справочников — прибор проверялся на этом
    # и был ослаблен до правды.
    ссылки = set(re.findall(r"references/([A-Za-z0-9_./-]+\.md)", body))
    каталог = skill_md.parent / "references"
    if каталог.is_dir():
        for файл in sorted(каталог.rglob("*.md")):
            имя = файл.relative_to(каталог).as_posix()
            if имя not in ссылки:
                bad.append("справочник не назван телом навыка и потому "
                           "недостижим: references/%s" % имя)
