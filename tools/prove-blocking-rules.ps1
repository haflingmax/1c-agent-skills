...
# check-1c-cli.py, зависящее от поведения платформы: и то, чем он блокирует,
# и то, о чём он только предупреждает, и то, что он обязан пропускать молча.
...
# ibcmd.exe поставляется с серверными компонентами, на этой машине его нет,
# проверять нечем — так и записано в спеке, раздел «Как проверяем».
#
...
$r = @(
  ...
  Run1C 'слитный-законный'    @('DESIGNER','/F',$Base,'/Lru','/DumpCfg','<CF>')
  Run1C 'слитный-выдуманный-рядом' @('DESIGNER','/F',$Base,'/Publish','/DumpCfg','<CF>')
  Run1C 'слитный-выдуманный-вместо' @('DESIGNER','/F',$Base,'/LoadCfgFromFile','<CF>')
