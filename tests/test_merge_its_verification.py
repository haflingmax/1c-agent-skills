...
def test_silence_must_say_what_was_searched():
    ...
    """
    d = файл(темы=[тема(итс="молчит", источник="", что_говорит="")])
    notes = mod.проверить(d, ["проверка прав доступа"], IDS)
    assert any("какими словами искали" in n for n in notes), notes


def test_silence_terms_may_live_in_either_field():
    """Перечень попыток годится и в «источник», и в «что_говорит».

    Агент по управляемым формам написал его вторым полем — «Искал „удаление
    формы", „DefaultForm", „AuxiliaryForm"…» — и первая редакция правила
    забраковала семнадцать честных записей подряд за раскладку по полям,
    а не за существо.
    """
    d = файл(темы=[тема(итс="молчит", источник="",
                        что_говорит="Искал «удаление формы», «DefaultForm» — не нашлось.")])
    assert mod.проверить(d, ["проверка прав доступа"], IDS) == []


@pytest.mark.parametrize("источник", [
    "pril7, раздел 7.4.4 «Конфигурация и расширения» (/DumpCfg)",
    "admin, раздел 6.8 «Загрузка информационной базы из файла»",
    "devguide, раздел 2.17.5.3",
    "metod8dev, статья 2290",
])
def test_its_base_with_section_number_is_a_source(источник):
    """Разделы, стоящие на приложении 7 и руководстве администратора,
    ссылаются номером раздела, а не главы.

    Первая редакция знала только «глава N» и забраковала восемь записей
    у `1c-build-and-db` — раздела, который целиком стоит на этих двух
    источниках и слова «глава» не употребляет вовсе.
    """
    d = файл(темы=[тема(источник=источник)])
    assert mod.проверить(d, ["проверка прав доступа"], IDS) == []
...
def test_silence_may_leave_what_it_says_empty():
    """Молчание нечего пересказывать — требовать «что_говорит» здесь неверно.

    Первая редакция требовала его безусловно и забраковала десять честных
    записей подряд: все они про инструменты чужого набора, о которых
    документация по платформе не говорит и говорить не может.
    """
    d = файл(темы=[тема(итс="молчит", что_говорит="",
                        источник="искал: «write_module_source», «MCP», «правка модуля»")])
    assert mod.проверить(d, ["проверка прав доступа"], IDS) == []


# --- обязательность: три уровня, а не два ----------------------------------

def test_recommendation_is_a_separate_level():
    """Стандарт требует, рекомендация советует — сливать их нельзя.

    Объявить обязательным то, что ИТС лишь советует, — та же ошибка,
    что и наоборот. Первая редакция схемы знала два значения, и агент
    по соглашениям о коде упёрся в это на пяти пробелах подряд, написав
    «стандарт (методическая рекомендация)» от руки.
    """
    d = файл(пробелы=[пробел(обязательность="рекомендация", источник="std724")])
    assert mod.проверить(d, ["проверка прав доступа"], IDS) == []


def test_recommendation_still_needs_a_standard_number():
    """Рекомендация живёт в стандарте — значит номер стандарта обязателен."""
    d = файл(пробелы=[пробел(обязательность="рекомендация",
                             источник="глава 4 руководства разработчика")])
    notes = mod.проверить(d, ["проверка прав доступа"], IDS)
    assert any("нет ни номера стандарта" in n for n in notes), notes
