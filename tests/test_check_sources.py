...
# --- Находка М-27: слово-маркер без разрешимой ссылки ----------------------
#
# Старая проверка ловила присутствие слова «руководство»/«глава»/«приложение»
# где угодно в абзаце. Абзац проходил, даже если не названы ни номер главы,
# ни номер приложения, ни конкретный документ — прочитать и проверить такую
# ссылку невозможно.


def test_bare_source_word_without_number_is_caught(tmp_path):
    f = tmp_path / "SKILL.md"
    f.write_text(
        "## Что здесь необратимо\n\n"
        "Необратимо изменение структуры данных согласно руководству, поэтому "
        "переносить эти операции без проверки нельзя вовсе.\n", encoding="utf-8")
    bad = mod.check_file(f)
    assert bad, "голое слово «руководству» без номера главы или ссылки не источник"


def test_numbered_source_passes(tmp_path):
    f = tmp_path / "SKILL.md"
    f.write_text(
        "## Что здесь необратимо\n\n"
        "Необратимо изменение структуры данных согласно приложению 7 "
        "руководства администратора 8.3.27, поэтому переносить без проверки нельзя.\n",
        encoding="utf-8")
    assert not mod.check_file(f)


def test_checked_by_run_without_pointer_is_caught(tmp_path):
    f = tmp_path / "SKILL.md"
    f.write_text(
        "## Что здесь необратимо\n\n"
        "Отчёт платформы об успехе источником не служит, это проверено запуском "
        "на живой базе несколько раз подряд без единого исключения.\n",
        encoding="utf-8")
    bad = mod.check_file(f)
    assert bad, "«проверено запуском» без ссылки на прогон/раздел не источник"


def test_checked_by_run_with_pointer_passes(tmp_path):
    f = tmp_path / "SKILL.md"
    f.write_text(
        "## Что здесь необратимо\n\n"
        "Отчёт платформы об успехе источником не служит: он бывает ложным даже "
        "при факте работы, проверено запуском (см. «Отчёт платформы бывает "
        "ложным» в [references/load-configuration.md](references/load-configuration.md)).\n",
        encoding="utf-8")
    assert not mod.check_file(f)
