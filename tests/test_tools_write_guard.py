...
READS_ONLY = {
    ...
    "check-quotes.py": "сверяет цитаты навыков с корпусом, только читает",
    "check-coverage.py": "сверяет подзаголовки руководства с навыком, только читает",
    "check-negations.py": "перепись утверждений об отсутствии, только читает",
    "read-acceptance.py":
        "читает журналы прогонов приёмки из каталога прогона, skills/ не трогает",
