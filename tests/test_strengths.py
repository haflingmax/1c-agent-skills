...
def test_core_names_collected_skills_inline():
    ...
    01.09.2026 образец расширен. Пока набор рос, строка начиналась словами
    «Собраны сейчас …»; с заполнением раскладки она стала «Собраны все
    пятнадцать навыков раздела: …». Проверяемое правило от этого не изменилось
    — состав назван внутри навыка и сверяется с каталогом, — поэтому расширен
    образец, а не ослаблена проверка: список имён по-прежнему обязан совпасть
    с `skills/` точь-в-точь.
    """
    ...
    # Абзац состава: от слов «Собран…» до пустой строки. Границей взят абзац,
    # а не парные звёздочки: пока набор рос, имена стояли внутри выделения,
    # а в заполненной раскладке выделен только зачин. Проверяемое от этого
    # не изменилось — список имён обязан совпасть с каталогом.
    m = re.search(r"\*\*Собран(?:ы)? (?:сейчас|все) .*?(?=\n\s*\n)",
                  text, re.S)
    assert m, "в ядре нет абзаца «Собраны …» с составом набора"
    named = set(re.findall(r"`([a-z0-9-]+)`", m.group(0)))
